...
## savedir
[ -z "${path_to_sft_checkpoint}" ] && path_to_sft_checkpoint=/savedir/adapter/ggi_llama13
[ -z "${path_to_predict_result}" ] && path_to_predict_result=/savedir/predict_results/ggi_llama13

## config
[ -z "${batch_size}" ] && batch_size=32
[ -z "${gradient_accumulation_steps}" ] && gradient_accumulation_steps=1
[ -z "${num_train_epochs}" ] && num_train_epochs=3
