## dataset
[ -z "${dataset}" ] && dataset=bivalent_unmethylated_train
...
## savedir
[ -z "${path_to_sft_checkpoint}" ] && path_to_sft_checkpoint=/savedir/adapter/bivalent_unmethylated_llama13
[ -z "${path_to_predict_result}" ] && path_to_predict_result=/savedir/predict_results/bivalent_unmethylated_llama13

# config
[ -z "${batch_size}" ] && batch_size=8
[ -z "${gradient_accumulation_steps}" ] && gradient_accumulation_steps=1
[ -z "${num_train_epochs}" ] && num_train_epochs=100
...
dataset=bivalent_unmethylated_test
